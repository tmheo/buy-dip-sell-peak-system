...
| 1.2.0 | 2026-01-20 | 허태명 | 백테스트 시각화 컴포넌트 추가, Phase 1 완료로 상태 변경 |
...
| 컴포넌트 | 파일명 | 용도 |
|----------|--------|------|
| PriceChart | `backtest/PriceChart.tsx` | 종가 + MA20/MA60 라인 차트 (로그 스케일, Recharts) |
| MetricsCharts | `backtest/MetricsCharts.tsx` | 6개 기술적 지표 미니 차트 (정배열, 기울기, 이격도, RSI, ROC, 변동성) |
| ProResultCard | `backtest/ProResultCard.tsx` | 전략별 결과 카드 - 자산 변동/MDD 차트, 성과 지표 표시 |
...
│   └── backtest/            # 백테스트 시각화 컴포넌트 (NEW)
│       ├── index.ts         # 컴포넌트 엔트리포인트
│       ├── PriceChart.tsx   # 가격 차트 (MA 포함)
│       ├── MetricsCharts.tsx # 기술적 지표 차트
│       └── ProResultCard.tsx # 전략 결과 카드
└── styles/
