...
Feature: 20일 연율화 변동성 계산
  ...
  Scenario: 변동성 계산
    ...
    And 최근 20일간 일별 수익률이 다음과 같다:
      ...
      | ... | ...    |
      | 20  | -0.02  |
    And 일별 수익률의 표준편차가 0.025이다
    When 변동성을 계산한다
    Then 연율화 변동성 = 0.025 × sqrt(252) × 100 = 39.69%이다

  Scenario: 낮은 변동성 (안정적 시장)
    Given 일별 수익률 표준편차가 0.01이다
    When 변동성을 계산한다
    Then 연율화 변동성은 약 15.87%이다

  Scenario: 높은 변동성 (불안정 시장)
    Given 일별 수익률 표준편차가 0.05이다
    When 변동성을 계산한다
    Then 연율화 변동성은 약 79.37%이다
```
