...
describe("buildDailyMetricRows", () => {
  it("골든 픽스처 전 구간에서 index 59부터 행을 만들고 값이 골든 값과 일치한다", () => {
    const rows = buildDailyMetricRows(FIXTURE_PRICES, FIXTURE_DATES, "FIXTURE", 0, lastIndex);
  ...
  it("startIndex가 59 미만이면 59로 보정한다 (MA60 최소 요구)", () => {
    const rows = buildDailyMetricRows(FIXTURE_PRICES, FIXTURE_DATES, "FIXTURE", 30, 70);
    expect(rows[0].date).toBe(FIXTURE_DATES[59]);
    expect(rows).toHaveLength(70 - 59 + 1);
  });

  it("배열 범위를 벗어난 인덱스 구간은 행을 만들지 않는다", () => {
    const rows = buildDailyMetricRows(FIXTURE_PRICES, FIXTURE_DATES, "FIXTURE", 59, lastIndex + 10);
    expect(rows).toHaveLength(lastIndex - 59 + 1);
    expect(rows[rows.length - 1].date).toBe(FIXTURE_DATES[lastIndex]);
  });
  ...
  it("필수 지표가 null인 날은 행을 스킵한다 (roc12 계산 불가)", () => {
    ...
    const rows = buildDailyMetricRows(prices, dates, "FIXTURE", 59, 69);
  ...
  it("시작 인덱스가 끝 인덱스보다 크거나 가격이 비면 빈 배열을 반환한다", () => {
    expect(buildDailyMetricRows(FIXTURE_PRICES, FIXTURE_DATES, "FIXTURE", 70, 60)).toEqual([]);
    expect(buildDailyMetricRows([], [], "FIXTURE", 0, 10)).toEqual([]);
  });
});
