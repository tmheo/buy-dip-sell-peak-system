...
분할 비율을 `R`로 정의한다. **2:1 분할이면 `R = 2`, 3:1이면 `R = 3`** (가격은 `1/R`로,
수량은 `R`배로 변한다). ex-date(권리락일)를 기준일로 삼는다.

| 대상 | 보정 규칙 | 비고 |
|------|-----------|------|
| **가격(`daily_prices`)** | 전체 재적재 권장. 수동 보정 시 분할 이전 모든 행의 `open/high/low/close/adjClose`를 `÷ R` | `volume`은 `× R`(선택), 정합성엔 영향 적음 |
...
- [ ] **신규 코드 없이 진행하는 임시 대안**:
  1. 해당 티커의 `daily_metrics` 행 삭제.
  2. 해당 티커의 `daily_prices` 행 삭제.
  3. `npx tsx src/index.ts init --ticker SOXL` 재실행 → Yahoo에서 분할 반영된 전체
     히스토리를 새로 적재(빈 테이블이므로 충돌 없음).
